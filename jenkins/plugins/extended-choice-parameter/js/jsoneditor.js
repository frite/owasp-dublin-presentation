...
JSONEditor.prototype = {
  ...
  getEditorRelativeTo: function(path, relativePath) {	
    var self = this;
	var result = null;
	
	if(relativePath && path) {
		var originalPathFragments = path.split(".");  
		var pathFragments = [].concat(originalPathFragments);
		var relativePathFragments = relativePath.split("/");
		for(var i=0;i<relativePathFragments.length; i++) {
			if(relativePathFragments[i] =="..") {
				pathFragments.pop();
			}else {
				var match = /\[([0-9]+)\]/.exec(relativePathFragments[i]);
				if(match != null) {
					pathFragments.push(originalPathFragments[match[1]]);
				}else {
					pathFragments.push(relativePathFragments[i]);
				}
			}
		}
		var newPath = pathFragments.join(".");
		result = self.getEditor(newPath);
	}
  ...
  watchAll:function(pathRegExp, callback) {
	  var self = this;
	  if(!this.editors) return;
	  $each(this.editors, function(path, e) {
		 if(pathRegExp.test(path)) {
  ...
  unwatchAll: function(pathRegExp, callback) {
	  var self = this;
	  if(!this.editors) return;
	  $each(this.editors, function(path, e) {
		 if(pathRegExp.test(path)) {
  ...
  notifyWatchers: function(path) {
    ...
    if(!this.watchlist || !this.watchlist[path]) return this;
    for(var i=0; i<this.watchlist[path].length; i++) {
      this.watchlist[path][i]({editor: self, path: path});
    }
  },
  isEnabled: function() {
...
JSONEditor.defaults.editors.objectarray = JSONEditor.AbstractEditor.extend({
  getDefault: function() {
    return this.schema["default"] || {};
  },
  register: function() {
  ...
  unregister: function() {
    ...
    if(this.rows) {
      $each(this.rows,function(key,row) {
		 self.rows[key].unregister();
	  });
    }
  },
  getNumColumns: function() {
    var info = this.getItemInfo(this.getItemKeys()[0]);
    // Tabs require extra horizontal space
    if(this.tabs_holder) {
      return Math.max(Math.min(12,info.width+2),4);
    }
    ...
    if(!this.options.compact) {
      ...
      if(this.schema.format === 'tabs') {
		...
		$each(this.defaultProperties, function(i,key) {
			...
			if(self.rows[key]) {
			  self.minwidth = Math.max(self.minwidth,(self.rows[key].options.grid_columns || self.rows[key].getNumColumns()));
			  self.maxwidth += (self.rows[key].options.grid_columns || self.rows[key].getNumColumns());
			}
		});
  ...
  getItemTitle: function() {
    if(!this.item_title) {
      this.item_title = this.schema.title || 'item';
    }
    return this.item_title;
  },
  getItemSchema: function(key) {
	if(this.schema.properties) {
		return $extend({},this.schema.properties[key]);
	}
    else {
      return {};
    }
  ...
  getItemInfo: function(key) {
    var schema = this.getItemSchema(key);
  ...
  getElementEditor: function(key) {
    var item_info = this.getItemInfo(key);
    var schema = this.getItemSchema(key);
    schema = this.jsoneditor.expandRefs(schema);
    schema.title = item_info.title || key;
  ...
  getMax: function() {
    return this.schema.properties.keys().length;
  },
  refreshNavigator: function(refresh_headers) {
    var self = this;
    $each(this.rows, function(key ,row) {
  ...
  setValue: function(value, initial) {
    ...
    $each(value,function(key ,val) {
      if(self.rows[key]) {
        self.rows[key].setValue(val,initial);
      }
      else if(self.row_cache[key]) {
        self.rows[key] = self.row_cache[key];
        self.rows[key].setValue(val,initial);
        self.rows[key].container.style.display = '';
        if(self.rows[key].tab) self.rows[key].tab.style.display = '';
        self.rows[key].register();
      }
      else {
        self.addRow(key, val, initial);
      }
    });
  ...
  refreshValue: function(force) {
    ...
    $each(this.rows,function(key,editor) {
      // Get the value for this editor
  ...
  addRow: function(key, value, initial) {
    var self = this;
    
	self.rows[key] = this.getElementEditor(key);
    self.row_cache[key] = self.rows[key];

    if(self.tabs_holder) {
      self.rows[key].tab_text = document.createElement('span');
      self.rows[key].tab_text.textContent = self.rows[key].getHeaderText();
      self.rows[key].tab = self.theme.getTab(self.rows[key].tab_text);
      self.rows[key].tab.addEventListener('click', function(e) {
        self.active_tab = self.rows[key].tab;
        self.refreshNavigator();
        e.preventDefault();
        e.stopPropagation();
      });

      self.theme.addTab(self.tabs_holder, self.rows[key].tab);
    }
    else if(self.dropdown_holder) {
      self.rows[key].dropdown_option = self.theme.getDropDownOption(self.rows[key].getHeaderText());
	  self.theme.addDropDownOption(self.dropdown_holder, self.rows[key].dropdown_option);
	}
	
    var controls_holder = self.rows[key].title_controls || self.rows[key].array_controls;
    
    if(value) self.rows[key].setValue(value, initial);
    self.refreshNavigator();
  },
  addControls: function() {
    var self = this;
...
JSONEditor.defaults.editors.arraySelectize = JSONEditor.AbstractEditor.extend({
  ...
  onWatchedFieldChange: function() {
    ...
    if(this.enumSource) {
	  ...
	  var new_value = [];
      for(var i=0; i<prev_value.length; i++)  {
		  if(select_options.indexOf(prev_value[i]) >=0) {
			  new_value.push(prev_value[i]);
		  }
	  }
	  
	  var selectize = window.jQuery(this.input)[0].selectize;
	  
		  
	  if(new_value.length != prev_value.length) {
		//this.input.value = new_value.join(",");
		selectize.clear(true);
		for(var i=0; i<select_options.length; i++) {
			  selectize.addOption({
				  text: select_titles[i],
				  value: select_options[i],
			  })
		} 
		selectize.refreshOptions(false);
		selectize.setValue(new_value, true);
...
JSONEditor.defaults.resolvers.unshift(function(schema) {
  if(schema.type === "object" && (schema.format == "tabs" || schema.format == "dropdown")) {
    return 'objectarray';
  }
});

JSONEditor.defaults.resolvers.unshift(function(schema) {
  if(schema.type === "array" && (schema.format == "selectize")) {
